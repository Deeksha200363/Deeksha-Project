  ...
  return (
    <article className="flex overflow-hidden flex-col px-6 pt-6 pb-10 bg-white rounded-xl min-w-[240px] w-[304px] max-md:px-5">
      <div className="flex gap-5 justify-between">
        ...
        <img loading="lazy" src={iconSrc} alt="" className="object-contain shrink-0 self-start w-6 aspect-square" />
      </div>
      <img loading="lazy" src={carImageSrc} alt={`${title} car`} className="object-contain self-center mt-16 aspect-square w-[60px] max-md:mt-10" />
      <div className="flex gap-5 justify-between mt-16 max-md:mt-10">
        {specs.map((spec, index) => (
          <div key={index} className="flex shrink-0 h-4 bg-indigo-50 rounded-3xl w-[68px]">{spec}</div>
        ))}
      </div>
      <div className="flex gap-6 mt-9">
        ...
        <button className="flex shrink-0 h-10 rounded-xl bg-neutral-700 w-[116px]" aria-label={buttonText}>
          {buttonText}
        </button>
      </div>
    </article>
